...
class IOClass:
    ...
    def create_data_file(self):
        ...
        if (restart==True):
            ...
            if (os.path.isfile(os.path.join(config2.data_path, 'restart', 'restart_'+timestamp+'.nc')) & (time_start != time_end)):
                self.GRID_RESTART = xr.open_dataset(os.path.join(config2.data_path, 'restart', 'restart_'+timestamp+'.nc'))
    ...
    def init_data_dataset(self):
        ...
        # Open input dataset
        self.DATA = xr.open_dataset(os.path.join(config2.data_path,config2.input_netcdf))
        ...
        if self.restart_date is None:
            print('--------------------------------------------------------------')
            print('\t Integration from %s to %s' % (config2.time_start, config2.time_end))
            print('--------------------------------------------------------------\n')
            self.DATA = self.DATA.sel(time=slice(config2.time_start, config2.time_end))   # Select dates from config.py
        else:
        ...
        if config2.time_start < start_interval:
            print('\n WARNING! Selected startpoint before first timestep of input data\n')
        if config2.time_end > end_interval:
            print('\n WARNING! Selected endpoint after last timestep of input data\n')
        if config2.time_start > end_interval or config2.time_end < start_interval:
