# -*- coding: utf-8 -*-
"""
Created on Sat Jan  9 14:46:30 2021
...
from config import *
import config2
sys.path.insert(1, './utilities/hpc')
...
def signals(self):
    self.BInput.clicked.connect(self.browse1)
    self.pushButton.clicked.connect(self.RunCosipy)
def browse1(self):
    filename = QFileDialog.getExistingDirectory()
    self.TInputFolder.setText(filename)
...
def run_cosipy(cluster, IO, DATA, RESULT, RESTART, futures):
    ...
    with Client(cluster) as client:
        ...
        if stake_evaluation is True:
            ...
            df_stat.to_csv(os.path.join(config2.data_path,'stake_statistics.csv'),sep='\t', float_format='%.2f')
            df_val.to_csv(os.path.join(config2.data_path,'stake_simulations.csv'),sep='\t', float_format='%.2f')
...
def compute_scale_and_offset(min, max, n):
    ...
    y=min.astype(np.float64)
    scale_factor = (x - y) / (2 ** n - 1)
    # translate the range to be symmetric about zero
    add_offset = x + 2 ** (n - 1) * scale_factor
...
def RunCosipy(self):
    ...
    time_start = str(date1.date())+"T"+date1.strftime("%H:%M")
    time_end = str(date2.date())+"T"+date2.strftime("%H:%M")
    ...
    data_path = str(self.TInputFolder.text())+"/"
    if data_path=="/":
        msg = QMessageBox()
        msg.setIcon(QMessageBox.Critical)
        msg.setText("Error")
        msg.setInformativeText('Enter a Valid COSIPY Input File Folder Path')
        msg.setWindowTitle("Error")
        msg.exec_()
        return
    
    # Zhadang example
    input_netcdf= str(self.TInputFilename.text())
    if input_netcdf=="":
        msg = QMessageBox()
        msg.setIcon(QMessageBox.Critical)
        msg.setText("Error")
        msg.setInformativeText('Enter a Valid COSIPY Input Filename ')
        msg.setWindowTitle("Error")
        msg.exec_()
        return
    output_netcdf = str(self.TOutputFilename.text())
    if output_netcdf=="":
        msg = QMessageBox()
        msg.setIcon(QMessageBox.Critical)
        msg.setText("Error")
        msg.setInformativeText('Enter a Valid Output Filename')
        msg.setWindowTitle("Error")
        msg.exec_()
        return
    f= open("./config2.py","w+")
    ...
    f.write("time_start_str=(time_start[0:10]).replace('-','')\n")
    f.write("time_end_str=(time_end[0:10]).replace('-','')\n")
    f.write("data_path = "+"'"+data_path+"'"+"\n")
    f.write("input_netcdf= "+"'"+input_netcdf+"'"+"\n")
    f.write("output_netcdf = "+"'"+output_netcdf+"'"+"\n")
    f.close()
    ...
    IO.get_result().to_netcdf(os.path.join(config2.data_path,config2.output_netcdf), encoding=encoding, mode = 'w')
    ...
    IO.get_restart().to_netcdf(os.path.join(config2.data_path,'restart_'+timestamp+'.nc'), encoding=encoding)
...
Ui_MainWindow.RunCosipy = RunCosipy
